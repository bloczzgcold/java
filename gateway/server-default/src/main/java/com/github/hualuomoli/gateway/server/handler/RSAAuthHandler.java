...
public class RSAAuthHandler implements AuthHandler {
	...
	@Override
	public AuthResponse deal(HttpServletRequest req, HttpServletResponse res, Partner partner, JSONParser jsonParser, BusinessHandler businessHandler, List<Filter> filters) throws Throwable {
		// 获取请求数据
		...
		for (Filter filter : filters) {
			filter.preHandle(rsaReq.partnerId, rsaReq.apiMethod, this, req, res);
		}
		...
		String result = null;
		try {
			result = businessHandler.handle(rsaReq.apiMethod, rsaReq.bizContent, jsonParser, req, res);
		} catch (Exception e) {
			ex = e;
		}
		// 业务后处理
		for (Filter filter : filters) {
			filter.postHandle(rsaReq.partnerId, rsaReq.apiMethod, this, businessHandler, req, res, ex);
