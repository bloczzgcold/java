...
public class MqSenderCratorTest extends ServiceTest {

  private static final String outputPath;
  private String packageName = "com.github.hualuomoli.sample.mq.sender.base";

  @Autowired
  private ReversService reversService;

  static {
    String path = MqSenderCratorTest.class.getResource("/").getPath();
    path = path.substring(0, path.lastIndexOf("/creator/target/"));
    outputPath = path + "/plugin/mq/sender";
